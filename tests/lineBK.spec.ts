...
import {firstGroupOccupation,secondGroupOccupation,thirdGroupOccupation} from '../fixtures/lineBK.ts'
...
firstGroupOccupation.forEach((data) => {
    test(`${data.testCaseOccupationName} with work experience ${data.testCaseExperienceName} and income ${data.testCaseIncome} at ${data.testCaseProvince}` , async({page}) => {
        ...
        //Check Bank Visible
        await bank.verifyTestcheckTwoBank(data.flow,'วงเงินให้ยืม LINE BK','วงเงินให้ยืมนาโน LINE BK')
    })
