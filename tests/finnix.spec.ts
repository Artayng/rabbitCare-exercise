...
import {firstGroupOccupation,secondGroupOccupation,thirdGroupOccupation} from '../fixtures/finnix.ts'
...
firstGroupOccupation.forEach((data) => {
    test(`${data.testCaseOccupationName} with work experience ${data.testCaseExperienceName} and income ${data.testCaseIncome}` , async({page}) => {
        ...
        //Check Bank Visible
        await bank.verifyTestcheck(data.flow,'สินเชื่อนาโนฟินนิกซ์ติดปีก')
    })
