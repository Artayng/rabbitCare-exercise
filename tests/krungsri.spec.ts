...
import {firstGroupOccupation,secondGroupOccupation} from '../fixtures/krungsri.ts'
...
firstGroupOccupation.forEach((data) => {
    test(`${data.testCaseOccupationName} with work experience ${data.testCaseExperienceName} and income ${data.testCaseIncome}` , async({page}) => {
        ...
        //Check Bank Visible
        await bank.verifyTestcheck(data.flow,'สินเชื่อหมุนเวียนส่วนบุคคล')
    })

})

secondGroupOccupation.forEach((data) => {
    test(`${data.testCaseOccupationName} with company registed ${data.testCaseBusinessRegis} and ${data.testCaseAvgRevenue} and income ${data.testCaseIncome} at ${data.testCaseProvince}`,async({page}) => {
        const bank = new bankFunction(page)
        await bank.onClickOTP()
